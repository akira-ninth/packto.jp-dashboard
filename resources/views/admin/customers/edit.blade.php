...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-4">
        <h1 class="h4 mb-0 fw-bold">顧客編集 — {{ $customer->subdomain }}</h1>
        <a href="{{ route('admin.customers.show', $customer) }}" class="btn btn-sm btn-alt-secondary">
            <i class="fa fa-arrow-left me-1"></i> 詳細に戻る
        </a>
    </div>

    <div class="block block-rounded">
        <div class="block-header block-header-default">
            <h3 class="block-title">顧客情報を編集</h3>
        </div>
        <div class="block-content">
            <form method="POST" action="{{ route('admin.customers.update', $customer) }}">
                ...
                <div class="mt-4">
                    ...
                    <a href="{{ route('admin.customers.show', $customer) }}" class="btn btn-alt-secondary ms-2">キャンセル</a>
                </div>
            </form>
        </div>
    </div>
